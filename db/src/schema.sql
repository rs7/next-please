...
CREATE TABLE `record` (
	...
	`date` TIMESTAMP NOT NULL,
	FOREIGN KEY (`user_id`) REFERENCES `user` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
	FOREIGN KEY (`record_book_id`) REFERENCES `record_book` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
	FOREIGN KEY (`reason_id`) REFERENCES `reason` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
);